...
func ParseDir(dir string) ([]Message, error) {
	...
	for _, entry := range entries {
		// Only active.<lang>.toml files are locales, matching the validator
		// and the //go:embed active.*.toml convention. A stray TOML (e.g. a
		// config file) must not pollute the generated keys.
